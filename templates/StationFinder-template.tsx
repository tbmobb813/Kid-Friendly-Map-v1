...
const YourCityStationFinder: React.FC<YourCityStationFinderProps> = ({
  ...
  const loadStations = async () => {
    try {
      ...
      const transformedStations: TransitStation[] = data.stations?.map((station: any) => ({
  ...
  const getStationIcon = (type: string, size: number = 20) => {
    switch (type) {
      case "bus": return <Bus size={size} color={Colors.primary} />;
      case "tram": return <Train size={size} color="#FF9800" />;
      case "ferry": return <MapPin size={size} color="#2196F3" />;
      case "multi": return <MapPin size={size} color="#9C27B0" />;
      default: return <Train size={size} color={Colors.primary} />;
    }
  };
        ...
        <View style={styles.filterSection}>
          ...
          <View style={styles.toggleFilters}>
            <TouchableOpacity
              style={[
                styles.toggleFilter,
                filters.accessibilityOnly && styles.activeToggleFilter
              ]}
              onPress={() => setFilters({ ...filters, accessibilityOnly: !filters.accessibilityOnly })}
            >
              <Accessibility size={16} color={filters.accessibilityOnly ? "#FFFFFF" : Colors.primary} />
              <Text style={[
            ...
            <TouchableOpacity
              style={[
                styles.toggleFilter,
                filters.transferHubsOnly && styles.activeToggleFilter
              ]}
              onPress={() => setFilters({ ...filters, transferHubsOnly: !filters.transferHubsOnly })}
            >
            ...
            <TouchableOpacity
              style={[
                styles.toggleFilter,
                filters.popularOnly && styles.activeToggleFilter
              ]}
