...
import { Train, Bus, Clock, MapPin, AlertTriangle, CheckCircle, RefreshCw } from "lucide-react-native";
...
const YourCityLiveArrivals: React.FC<YourCityLiveArrivalsProps> = ({
  ...
  const fetchArrivals = async () => {
    ...
    try {
      ...
      const transformedArrivals: ArrivalPrediction[] = data.arrivals?.map((arrival: any) => ({
        ...
        arrivalTime: Math.round((new Date(arrival.REPLACE_TIME_FIELD).getTime() - Date.now()) / 60000),
  ...
  const getVehicleIcon = (type: string, size: number = 16) => {
    switch (type) {
      case "bus": return <Bus size={size} color="#666666" />;
      case "tram": return <Train size={size} color="#666666" />;
      case "ferry": return <MapPin size={size} color="#666666" />;
      default: return <Train size={size} color="#666666" />;
    }
  };
