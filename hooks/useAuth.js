...
export const [AuthProvider, useAuth] = createContextHook(() => {
    ...
    const logout = useCallback(async () => {
        ...
        try {
            ...
            log.error('Logout error in context', error);
        }
    }, []);
    ...
    const verifyParentalPin = useCallback(async (pin) => {
        return authManager.verifyParentalPin(pin);
    }, []);
    const setParentalPin = useCallback(async (pin) => {
    ...
    return {
        ...
        toggleLikedSuggestion: (id, liked) => authManager.toggleLikedSuggestion(id, liked),
        saveRoute: (routeId, save) => authManager.saveRoute(routeId, save),
        // Utilities
        ...
        canAccessFeature: (feature) => {
            if (!authState.user?.parentalControls?.isEnabled)
                return true;
...
export function useAuthActions() {
    const { login, register, logout, updateProfile, changePassword, resetPassword, verifyParentalPin, setParentalPin } = useAuth();
    return {
        login,
        register,
        logout,
...
export function useParentalControls() {
    const { user, hasParentalControls, canAccessFeature, verifyParentalPin, setParentalPin, isParentUser, isChildUser } = useAuth();
    return {
        hasParentalControls,
        canAccessFeature,
        verifyParentalPin,
