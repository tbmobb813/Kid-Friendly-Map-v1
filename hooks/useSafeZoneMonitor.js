...
export const useSafeZoneMonitor = () => {
    ...
    const [safeZoneStates, setSafeZoneStates] = useState({});
    ...
    useEffect(() => {
        if (currentLocation && safeZones.length > 0) {
            ...
            safeZones.forEach(zone => {
                if (zone.isActive) {
                    const distance = calculateDistance(currentLocation.latitude, currentLocation.longitude, zone.latitude, zone.longitude);
    ...
    const sendSafeZoneNotification = async (event) => {
        if (!settings.safeZoneAlerts)
            return;
    ...
    const checkSafeZones = async (location) => {
        const newStates = {};
        const events = [];
        safeZones.forEach(zone => {
            if (!zone.isActive)
        ...
        try {
            ...
            if (Platform.OS !== 'web') {
                ...
                if (foregroundStatus !== 'granted') {
                    Alert.alert('Location Permission Required', 'Safe zone monitoring requires location access to work properly.', [{ text: 'OK' }]);
                    return;
                }
