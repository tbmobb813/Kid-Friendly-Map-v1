...
export function useApiWithErrorHandling(initialData = null, options = {}) {
    ...
    const { showToastOnError = true, showToastOnSuccess = false, successMessage = 'Operation completed successfully', retryable = true } = options;
    const execute = useCallback(async (apiCall, customSuccessMessage) => {
...
export function usePaginatedApiWithErrorHandling(options = {}) {
    const [paginationState, setPaginationState] = useState({
        page: 1,
        hasMore: true,
        totalCount: 0
    });
    const apiState = useApiWithErrorHandling([], options);
