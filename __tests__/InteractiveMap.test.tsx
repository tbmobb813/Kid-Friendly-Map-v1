...
jest.mock('../components/ExpoMapView', () => ({ __esModule: true, default: (props: any) => <View {...props} /> }));
jest.mock('../components/MapLibreRouteView', () => ({ __esModule: true, default: (props: any) => <View {...props} /> }));
jest.mock('../components/MapPlaceholder', () => ({ __esModule: true, default: (props: any) => <View {...props} /> }));
...
describe('InteractiveMap', () => {
  ...
  it('shows transit info when showTransitStations is true', () => {
    const { getByText } = render(
      <InteractiveMap origin={origin} destination={destination} route={route} showTransitStations={true} />
    );
  ...
  it('calls onSelectLocation when map is tapped (web fallback)', () => {
    ...
    const { getByTestId } = render(
      <InteractiveMap origin={origin} destination={destination} route={route} testId="test-map" onSelectLocation={onSelectLocation} />
    );
  ...
  it('engine detection prefers native when available', () => {
    ...
    const { getByTestId } = render(
      <InteractiveMap origin={origin} destination={destination} route={route} testId="test-map" preferNative={true} />
    );
    expect(getByTestId('test-map')).toBeTruthy();
  });
