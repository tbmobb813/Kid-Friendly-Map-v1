...
const overrideCenter = {
  latitude: ensureNumber(
    process.env.EXPO_PUBLIC_MAP_DEFAULT_LAT,
    baseCenter.latitude
  ),
  longitude: ensureNumber(
    process.env.EXPO_PUBLIC_MAP_DEFAULT_LNG,
...
const routingExtras = {
  baseUrl:
    typeof orsBaseUrlFromEnv === "string" && orsBaseUrlFromEnv.length > 0
      ? orsBaseUrlFromEnv
      : (baseConfig.extra as any)?.routing?.baseUrl ??
...
const config: ExpoConfig = {
  ...
  userInterfaceStyle:
    baseConfig.userInterfaceStyle ?? baseConfig.userInterfaceStyle,
