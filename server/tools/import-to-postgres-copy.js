...
function runPsql(sql) {
  const res = spawnSync('psql', ['-v', 'ON_ERROR_STOP=1', process.env.DATABASE_URL, '-c', sql], { stdio: 'inherit' });
...
try {
  ...
  // Run COPY commands
  runPsql(`\copy routes(route_id, route_short_name, route_long_name, route_type) FROM '${path.join(tmpDir,'routes.csv')}' WITH (FORMAT csv)`);
  runPsql(`\copy trips(trip_id, route_id, service_id, trip_headsign) FROM '${path.join(tmpDir,'trips.csv')}' WITH (FORMAT csv)`);
  runPsql(`\copy stops(stop_id, stop_name, stop_lat, stop_lon) FROM '${path.join(tmpDir,'stops.csv')}' WITH (FORMAT csv)`);
  runPsql(`\copy stop_times(trip_id, stop_sequence, stop_id, arrival_time, departure_time) FROM '${path.join(tmpDir,'stop_times.csv')}' WITH (FORMAT csv)`);
