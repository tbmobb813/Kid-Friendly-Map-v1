...
const mock = {
  getTrip: async (tripId) => {
    if (tripId === 'trip1' || tripId === 'mta-subway-1' || tripId === 'trip1') return { trip_id: 'trip1', trip_headsign: 'South Ferry' };
    return null;
  },
  getNextStopsForTrip: async (tripId, count) => {
    if (tripId === 'trip1' || tripId === 'mta-subway-1') return [{ stop_id: 's1', stop_name: 'Times Sq-42nd St' }];
    return [];
  }
};
...
(async () => {
  ...
  try {
    ...
    assert(r.destination === 'South Ferry', `expected destination South Ferry, got ${r.destination}`);
    assert(r.nextStopName === 'Times Sq-42nd St', `expected nextStopName Times Sq-42nd St, got ${r.nextStopName}`);
