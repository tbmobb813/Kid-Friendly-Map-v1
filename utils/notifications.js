...
const inAppListeners = [];
export const addInAppBannerListener = (cb) => {
    inAppListeners.push(cb);
    return () => {
        const i = inAppListeners.indexOf(cb);
        if (i > -1)
            inAppListeners.splice(i, 1);
    };
};
export const showInAppBanner = (banner) => {
...
export const showDevelopmentBuildRecommendation = () => {
    if (isExpoGo) {
        Alert.alert('📱 Enhanced Features Available', 'For the best experience with notifications and background features, consider using a development build instead of Expo Go.\n\nLearn more at: docs.expo.dev/develop/development-builds/', [
