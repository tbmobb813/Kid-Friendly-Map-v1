...
export const useRegionStore = create()(persist((set, get) => ({
    availableRegions: [
        newYorkConfig,
        chicagoConfig,
        sanFranciscoConfig,
    ...
    updateRegionTransitData: (regionId, transitData) => set((state) => ({
        availableRegions: state.availableRegions.map(region => region.id === regionId
            ? { ...region, ...transitData }
            : region),
    ...
    searchRegions: (query) => {
        ...
        return regions.filter(region => region.name.toLowerCase().includes(lowercaseQuery) ||
