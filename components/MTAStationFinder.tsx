...
import { Search, MapPin, Train, Bus, Navigation, Accessibility, Clock, Star, Info } from "lucide-react-native";
...
const MTAStationFinder: React.FC<MTAStationFinderProps> = ({ 
  userLocation,
  onStationSelect 
}) => {
  ...
  const mockStations: StationInfo[] = [
    {
      ...
      kidFriendlyInfo: {
        ...
        whatToSee: ["Bright lights everywhere", "Street performers", "Red Steps", "TKTS booth", "Costume characters"],
        tip: "This is one of the busiest stations in the world - hold hands tight and stay close!",
        funFact: "Over 200,000 people pass through this station every day - that's like a whole city!",
        safetyNote: "Very crowded - always stay with your adult and watch for moving crowds"
      },
      exits: [
        { name: "7th Av & 42nd St", description: "Main Times Square exit with red steps", isAccessible: true },
        { name: "Broadway & 42nd St", description: "Theater District exit near TKTS", isAccessible: true },
        { name: "8th Av & 42nd St", description: "Port Authority Bus Terminal exit", isAccessible: false }
      ],
      amenities: ["Police station", "Information booth", "Restrooms", "Food vendors"],
      nearbyAttractions: ["Times Square", "Broadway theaters", "M&M Store", "Hershey's Chocolate World"]
    },
    {
      id: "central-park-59",
      ...
      kidFriendlyInfo: {
        ...
        whatToSee: ["Central Park entrance", "Columbus statue", "Horse carriages", "Street artists"],
      ...
      exits: [
        { name: "Central Park South", description: "Direct entrance to Central Park", isAccessible: true },
        { name: "Columbus Circle", description: "Shopping center and restaurants", isAccessible: true }
      ],
    ...
    },
    {
      ...
      exits: [
        { name: "Park Row", description: "Brooklyn Bridge pedestrian entrance", isAccessible: false },
        { name: "Centre St", description: "City Hall and government buildings", isAccessible: false }
      ],
      amenities: ["Historic tile work", "Tourist signs"],
      nearbyAttractions: ["Brooklyn Bridge", "South Street Seaport", "Stone Street", "9/11 Memorial"]
    },
    {
      id: "14th-st-union-sq",
      ...
      exits: [
        { name: "Union Square Park", description: "Main park entrance with farmers market", isAccessible: true },
    ...
    },
    {
      ...
      exits: [
        { name: "1st Avenue", description: "Main avenue with shops and restaurants", isAccessible: true }
      ],
          ...
          {station.lines.map(line => (
            <View 
              ...
              style={[
                ...
                { backgroundColor: station.type === "subway" ? subwayLineColors[line] || Colors.primary : Colors.primary }
              ]}
            >
      ...
      <View style={styles.attractionsSection}>
        ...
        <View style={styles.attractionsList}>
          {station.kidFriendlyInfo.whatToSee.slice(0, 3).map((attraction, index) => (
            <Text key={index} style={styles.attractionItem}>• {attraction}</Text>
          ))}
          {station.kidFriendlyInfo.whatToSee.length > 3 && (
            <Text style={styles.moreAttractions}>and {station.kidFriendlyInfo.whatToSee.length - 3} more...</Text>
          )}
        </View>
      </View>
