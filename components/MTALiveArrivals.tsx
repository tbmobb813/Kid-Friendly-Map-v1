...
import { Train, Bus, Clock, MapPin, AlertTriangle, Info, RefreshCw, Star } from "lucide-react-native";
...
const MTALiveArrivals: React.FC<MTALiveArrivalsProps> = ({
  ...
  const mockSubwayArrivals: ArrivalInfo[] = [
    {
      id: "1", route: "1", destination: "South Ferry", direction: "Downtown",
      arrivalTime: 2, status: "approaching", track: "1",
      kidNote: "Train to downtown Manhattan and the Statue of Liberty!"
    },
    {
      id: "2", route: "N", destination: "Coney Island", direction: "Brooklyn-bound", 
      arrivalTime: 4, status: "on-time", track: "4",
      kidNote: "Train to the beach and amusement park!"
    },
    {
      id: "3", route: "Q", destination: "96th St", direction: "Uptown",
      arrivalTime: 6, status: "on-time", track: "3",
      kidNote: "Train to the Upper East Side"
    },
    {
      id: "4", route: "4", destination: "Woodlawn", direction: "Uptown",
      arrivalTime: 8, status: "delayed", track: "2",
      kidNote: "Express train to the Bronx", delayReason: "Signal problems ahead"
    },
    {
      id: "5", route: "7", destination: "Flushing", direction: "Queens-bound",
      arrivalTime: 10, status: "on-time", track: "5",
  ...
  const mockBusArrivals: ArrivalInfo[] = [
    {
      id: "1", route: "M42", destination: "UN/1st Av", direction: "Eastbound",
      arrivalTime: 3, status: "on-time",
      kidNote: "Bus across 42nd Street to the United Nations"
    },
    {
      id: "2", route: "M42", destination: "Port Authority", direction: "Westbound",
      arrivalTime: 7, status: "on-time", 
      kidNote: "Bus to the big bus terminal"
    },
    {
      id: "3", route: "M104", destination: "42nd St/Broadway", direction: "Southbound",
      arrivalTime: 5, status: "delayed", delayReason: "Heavy traffic"
    }
  ];

  const mockAlerts: StationAlert[] = [
    {
      id: "1", type: "information", severity: "low",
      message: "Elevator service normal", 
      kidFriendlyMessage: "Good news! The elevator is working for people who need it 🛗",
      affectedRoutes: ["ALL"]
    },
    {
      id: "2", type: "delay", severity: "medium",
    ...
    <View key={arrival.id} style={styles.arrivalCard}>
      <View style={styles.arrivalHeader}>
        <View style={styles.routeInfo}>
          {stationType === "subway" ? (
            <View style={[styles.routeIndicator, { backgroundColor: getRouteColor(arrival.route) }]}>
          ...
          <View style={styles.destinationInfo}>
            ...
            {arrival.track && (
              <Text style={styles.trackText}>Track {arrival.track}</Text>
            )}
          </View>
        </View>
    ...
    <View key={alert.id} style={[styles.alertCard, styles[`alert${alert.severity.charAt(0).toUpperCase() + alert.severity.slice(1)}`]]}>
      <View style={styles.alertHeader}>
        ...
        <Text style={styles.alertTitle}>
          {alert.type === "delay" ? "Service Delay" : 
           alert.type === "service-change" ? "Service Change" : "Information"}
        </Text>
      </View>
      
      <Text style={styles.alertMessage}>
        {alert.kidFriendlyMessage || alert.message}
      </Text>
      ...
      {alert.affectedRoutes.length > 0 && alert.affectedRoutes[0] !== "ALL" && (
        <View style={styles.affectedRoutes}>
          ...
          {alert.affectedRoutes.map((route) => (
            <View key={route} style={[styles.affectedRouteTag, { backgroundColor: getRouteColor(route) }]}>
