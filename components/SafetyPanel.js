...
const SafetyPanel = ({ currentLocation, currentPlace }) => {
        ...
        try {
            ...
            Alert.alert("Emergency Call", "Do you need to call for help?", [
                { text: "Cancel", style: "cancel" },
                {
                    ...
                    onPress: async () => {
                        try {
                            await withRetry(() => Linking.openURL("tel:911"), DEFAULT_RETRY_CONFIG.critical, 'Emergency 911 call');
                ...
                },
                {
                    ...
                    onPress: async () => {
                        try {
                            const primaryContact = settings.emergencyContacts.find(c => c.isPrimary) || safetyContacts.find(c => c.isPrimary);
                            if (primaryContact) {
                                await withRetry(() => Linking.openURL(`tel:${primaryContact.phone}`), DEFAULT_RETRY_CONFIG.critical, 'Parent emergency call');
                                log.info('Parent emergency call initiated successfully', { contactId: primaryContact.id });
                            }
                            else {
    ...
    const handleShareLocation = async () => {
        try {
            ...
            if (primaryContact && Platform.OS !== 'web') {
                await withRetry(() => Linking.openURL(`sms:${primaryContact.phone}&body=${encodeURIComponent(message)}`), DEFAULT_RETRY_CONFIG.critical, 'Share location SMS');
            ...
            try {
                await withRetry(() => Promise.resolve(updateLastKnownLocation({
                    latitude: currentLocation.latitude,
                    longitude: currentLocation.longitude,
                    timestamp: Date.now(),
                    placeName: currentPlace?.name,
                })), DEFAULT_RETRY_CONFIG.storage, 'Update last known location');
            }
            catch (error) {
            ...
            { text: "Not now", style: "cancel" },
            {
                ...
                onPress: () => {
                    ...
                    if (primaryContact && Platform.OS !== 'web') {
                        ...
                        Linking.openURL(`sms:${primaryContact.phone}&body=I made it to ${placeName} safely! 😊`);
                    }
                    // Update last known location in parent dashboard
    ...
    const handlePhotoCheckIn = async () => {
        try {
            ...
            if (!currentLocation) {
                const locationError = handleLocationError({ code: 'POSITION_UNAVAILABLE' });
                Alert.alert('Location Required', locationError.userMessage + '\n\n' + (locationError.suggestedAction || ''));
            ...
            if (Platform.OS === 'web') {
                ...
                try {
                    await withRetry(() => Promise.resolve(addLocationVerifiedPhotoCheckIn(webCheckIn, currentLocation, placeCoordinates)), DEFAULT_RETRY_CONFIG.storage, 'Web photo check-in');
            ...
            try {
                const permissionResult = await withRetry(() => ImagePicker.requestCameraPermissionsAsync(), { maxAttempts: 2, delayMs: 500 }, 'Camera permission request');
                if (!permissionResult.granted) {
                    const cameraError = handleCameraError({ message: 'Camera permission denied' });
                    if (Platform.OS === 'android') {
                        Alert.alert("Camera Permission", cameraError.userMessage + "\n\nPlease enable camera access in your device settings.", [
                            { text: "Cancel", style: "cancel" },
            ...
            try {
                const result = await withRetry(() => ImagePicker.launchCameraAsync(cameraOptions), { maxAttempts: 2, delayMs: 1000 }, 'Camera launch');
                if (!result.canceled && result.assets[0]) {
                    ...
                    if (!checkInValidation.isValid) {
                        log.error('Invalid photo check-in data', undefined, { errors: checkInValidation.errors });
                        showToast('Check-in data is invalid', 'error');
                        return;
                    }
                    ...
                    let verification;
                    try {
                        verification = await withRetry(() => Promise.resolve(addLocationVerifiedPhotoCheckIn(checkInData, currentLocation, placeCoordinates)), DEFAULT_RETRY_CONFIG.storage, 'Photo check-in verification');
                    }
                    catch (error) {
                    ...
                    try {
                        await withRetry(() => Promise.resolve(addCheckInToDashboard({
                            id: `checkin_${Date.now()}`,
                            timestamp: Date.now(),
                            placeName: currentPlace.name,
                            photoUrl: result.assets[0].uri,
                            location: currentLocation,
                        })), DEFAULT_RETRY_CONFIG.storage, 'Add check-in to dashboard');
                    }
                    catch (error) {
                    ...
                    if (verification.isWithinRadius) {
                        Alert.alert("✅ Check-in Verified!", `Great job! You're ${accuracyDescription} (${distanceText}) from ${currentPlace.name}. Your location has been verified! 📸`, [{ text: "Awesome!", style: "default" }]);
                    }
                    else {
                        Alert.alert("⚠️ Location Check", `Photo taken! However, you appear to be ${distanceText} from ${currentPlace.name}. Make sure you're at the right location for accurate check-ins.`, [{ text: "Got it", style: "default" }]);
                    }
    ...
    return (_jsx(ErrorBoundary, { children: _jsxs(View, { style: styles.container, children: [_jsxs(Pressable, { style: styles.header, onPress: () => setIsExpanded(!isExpanded), children: [_jsx(Shield, { size: 20, color: Colors.primary }), _jsx(Text, { style: styles.title, children: "Safety Tools" }), _jsx(Text, { style: styles.expandIcon, children: isExpanded ? "−" : "+" })] }), isExpanded && (_jsxs(View, { style: styles.content, children: [_jsxs(View, { style: styles.buttonRow, children: [_jsxs(Pressable, { style: styles.safetyButton, onPress: handleEmergencyCall, children: [_jsx(Phone, { size: 18, color: "#FFFFFF" }), _jsx(Text, { style: styles.buttonText, children: "Emergency" })] }), _jsxs(Pressable, { style: styles.safetyButton, onPress: handleShareLocation, children: [_jsx(MapPin, { size: 18, color: "#FFFFFF" }), _jsx(Text, { style: styles.buttonText, children: "Share Location" })] })] }), _jsxs(View, { style: styles.buttonRow, children: [_jsxs(Pressable, { style: styles.safetyButton, onPress: handleSafeArrival, children: [_jsx(MessageCircle, { size: 18, color: "#FFFFFF" }), _jsx(Text, { style: styles.buttonText, children: "I Made It!" })] }), _jsxs(Pressable, { style: [styles.safetyButton, isPhotoLoading && styles.loadingButton], onPress: handlePhotoCheckIn, disabled: isPhotoLoading, children: [_jsx(Camera, { size: 18, color: "#FFFFFF" }), _jsx(Text, { style: styles.buttonText, children: isPhotoLoading ? "Taking Photo..." : "Photo Check-in" })] })] }), _jsxs(View, { style: styles.tipContainer, children: [_jsx(AlertTriangle, { size: 16, color: Colors.warning }), _jsx(Text, { style: styles.tipText, children: "Always stay with a trusted adult when traveling" })] })] }))] }) }));
};
