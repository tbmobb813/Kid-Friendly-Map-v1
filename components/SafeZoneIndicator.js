...
export const SafeZoneIndicator = () => {
    ...
    const getStatusColor = () => {
        if (!isMonitoring)
            return '#F59E0B'; // Orange for inactive
        if (status && status.inside.length > 0)
            return '#10B981'; // Green for inside safe zone
        return '#EF4444'; // Red for outside safe zones
    };
    const getStatusText = () => {
        if (!isMonitoring)
            return 'Safe zone monitoring inactive';
        if (!status)
    ...
    return (_jsxs(View, { style: [styles.container, { borderLeftColor: statusColor }], children: [_jsxs(View, { style: styles.content, children: [_jsx(Shield, { size: 16, color: statusColor }), _jsx(Text, { style: styles.statusText, children: getStatusText() })] }), _jsx(View, { style: [styles.indicator, { backgroundColor: statusColor }] })] }));
};
